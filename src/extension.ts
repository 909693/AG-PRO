...
// Seamless Switch
import { seamlessSwitchService } from './seamless_switch';
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    logger.info(`AG Pro v${version} - Systems Online`);
    ...
    // 注册无感切号命令 (Ctrl+Shift+A)
    context.subscriptions.push(
        vscode.commands.registerCommand('agCockpit.seamlessSwitch', async () => {
            await seamlessSwitchService.showSwitchQuickPick();
        }),
    );
    ...
    // 连接 Cockpit Tools WebSocket (可选，连不上不影响核心功能)
    try {
        cockpitToolsWs.connect();
    } catch {
        logger.info('[Startup] Cockpit Tools WebSocket not available (optional)');
    }
    ...
    logger.info('AG Pro Fully Operational');
}
...
export async function deactivate(): Promise<void> {
    logger.info('AG Pro: Shutting down...');
