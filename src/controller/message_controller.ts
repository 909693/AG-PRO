        ...
        this.hud.onSignal(async (msg: WebviewMessage) => {
            ...
            switch (message.command) {
                ...
                case 'autoTrigger.switchLoginAccount':
                    // 切换登录账户 —— 优先使用无感切号（AG Pro）
                    if (message.email) {
                        logger.info(`User switching login account to: ${message.email}`);

                        try {
                            // 优先：无感切号（通过补丁注入的命令）
                            const { seamlessSwitchService } = await import('../seamless_switch');
                            const success = await seamlessSwitchService.switchToAccount(message.email);
                            if (success) {
                                logger.info(`[MsgCtrl] Seamless switch to ${message.email} successful`);
                                // 刷新 webview UI 状态
                                this.hud.sendMessage({ type: 'accountSwitched', email: message.email });
                                this.hud.sendMessage({ type: 'refreshAccounts' });
                                if (this.refreshService) {
                                    await this.refreshService.refresh({ reason: 'seamlessSwitch' });
                                }
                                break;
                            }
                            logger.info('[MsgCtrl] Seamless switch not ready, falling back to WebSocket...');
                        } catch (err) {
                            logger.debug(`[MsgCtrl] Seamless switch failed: ${err}, trying WebSocket...`);
                        }

                        // Fallback: WebSocket 切号
                        if (!cockpitToolsWs.isConnected) {
                            const connected = await cockpitToolsWs.waitForConnection(5000);
                            if (!connected) {
                                vscode.window.showWarningMessage(
                                    'Seamless switch not available. Apply patch first (Ctrl+Shift+A).',
                                );
                                return;
                            }
                        }
                        ...
                        try {
                            ...
                            vscode.window.showErrorMessage(`切换失败: ${err.message}`);
                        }
                    } else {
                ...
                case 'switchAccount': // Alias for switching client account
                    if (typeof message.email === 'string') {
                        ...
                        try {
                            const { seamlessSwitchService } = await import('../seamless_switch');
                            const success = await seamlessSwitchService.switchToAccount(email);
                            if (success) {
                                this.hud.sendMessage({
                                    type: 'actionResult',
                                    data: { status: 'success', message: t('seamless.switchSuccess', { email }) || `账号切换成功: ${email}` },
                                });
                                this.hud.sendMessage({ type: 'accountSwitched', email });
                                this.hud.sendMessage({ type: 'refreshAccounts' });
                                if (this.refreshService) {
                                    await this.refreshService.refresh({ reason: 'seamlessSwitch' });
                                }
                            ...
                            logger.debug(`[MsgCtrl] Seamless switch fallback: ${err}`);
                        }
                        ...
                        if (!cockpitToolsWs.isConnected) {
                            ...
                            if (!connected) {
                                vscode.window.showWarningMessage(
                                    'Seamless switch not ready. Use Ctrl+Shift+A to apply patch first.',
                                );
                                return;
                            }
                        }
